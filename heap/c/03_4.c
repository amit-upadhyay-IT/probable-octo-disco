...
int main()
{
    ...
    while (k--)
    {
        struct AVLnode *temp = getInorderSuccessor(root);
        printf("%d ", temp->data);
        root = remove_node(root, temp->data);
    }
    printf("\n");
    return 0;
}

// time complexity = O(nlogn) + O(k*2logn)
// space complexity = O(n), for storing the nodes in avl tree
